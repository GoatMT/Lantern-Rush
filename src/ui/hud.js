...
export class HUD{
  ...
  update(dt){
    ...
    $('controlled-profile').textContent=(p.data.overall??'—')+' OVR · '+(p.data.playstyle?.label||'');
    ...
    for(const player of m.players){
      ...
      const show=!player.sentOff&&(intro?m.phaseTime>=9.8:player===m.controlled);label.hidden=!show;if(!show)continue;
      label.classList.toggle('you',player.team===0);
      label.textContent=(intro?player.role+' · ':player.team===0?'YOU · ':'')+player.name+(player.jersey!==null?' #'+player.jersey:'');
      const pos=this.app.renderer.project(player.x,3.4,player.z);label.hidden=!pos.visible;label.style.left=pos.x+'px';label.style.top=pos.y+'px';
    }
