...
export function performSkill(match,p,aim=null){
  ...
  options=options.filter(k=>k!==p.lastSkill);if(!options.length)options=['body-feint'];
  ...
  p.skillPlan={kind,x:n.x,z:n.z,burst,time:kind==='roulette'?.7:.55};p.lastSkill=kind;p.skill=.55;p.cooldown=1.15;
  p.animate(kind,.65,{side,aim:desired});match.charge=0;match.cancelShotUntilRelease=kind==='fake-shot';
  ...
  if(match.random()<failure){
    ...
    match.ball.vx=n.x*(speed+1.5);match.ball.vz=n.z*(speed+1.5);
  }
