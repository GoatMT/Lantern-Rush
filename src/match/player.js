...
export class Player {
  ...
  reset(direction){
    ...
    this.action=null;this.skillPlan=null;this.skill=0;this.animationTime=0;this.animation=this.role==='GK'?'ready':'idle';this.locomotion='idle';this.keeperState={};this.gait=0;
  }
  move(dx,dz,intensity,dt,sprint=false,facing=null){
    ...
    const speed=(boosting?PLAY.sprintSpeed:PLAY.runSpeed)*this.attributes.speed*(.88+.12*this.stamina)*clamp(intensity,0,1);
    ...
    const rate=!moving?PLAY.braking:alignment<-.2?(boosting?17:25):boosting?PLAY.sprintAcceleration:PLAY.acceleration;
    ...
    let desired=facing?normalize(facing.x-this.x,facing.z-this.z):actualSpeed>.2?normalize(this.vx,this.vz):{x:this.faceX,z:this.faceZ};
    if(this.action?.context?.aim&&this.action.time<this.action.duration*.6)desired=this.action.context.aim;
    const delta=angleDelta(oldAngle,Math.atan2(desired.x,desired.z)),angle=oldAngle+clamp(delta,-(boosting?3.8:7)*dt,(boosting?3.8:7)*dt);
